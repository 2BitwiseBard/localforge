...
class CodeWatcher(BaseAgent):
    ...
    async def run(self):
        ...
        for directory in directories:
            ...
            result = await self.call_tool("git_context", {
                "directory": dir_path,
                "include_diff": True,
                "log_count": 5,
            }, timeout=30)
            ...
            # Review the changes
            review = await self.call_tool("review_diff", {
                "diff": text,
                "focus": focus,
            }, timeout=90)
            ...
            issue_words = ["issue", "bug", "vulnerability", "error", "warning",
                           "critical", "security", "unsafe", "unwrap"]
            ...
            await self.send_message("code.changes_detected", {
                "directory": dir_path,
                "agent": self.agent_id,
            })
